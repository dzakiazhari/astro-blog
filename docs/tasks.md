...
| Bug Fix | Scroll manager emitted `null` progress snapshots on first navigation, breaking the back-to-top button and progress bar. | Subscriptions now fall back to the cached snapshot and guard against empty detail payloads, so SPA transitions keep helpers running. | `src/scripts/scroll-manager.ts`, `src/scripts/back-to-top.ts`, `src/scripts/reading-progress.ts` |
| Refactor | Scroll helpers still relied on global window singletons and inline public scripts, causing race conditions after client navigations. | Bundled the scroll manager and UI hooks as Astro modules with lifecycle-aware setup so progress indicators survive SPA swaps. | `src/layouts/Layout.astro`, `src/components/ScrollEnhancements.astro`, `src/scripts/scroll-manager.ts`, `src/scripts/back-to-top.ts`, `src/scripts/reading-progress.ts` |
