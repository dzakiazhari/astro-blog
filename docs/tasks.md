...
| Content | Featured post for the Pages CMS article stayed hidden on the homepage and posts index because its publish timestamp was saved in JST but encoded as UTC, so the scheduling filter considered it unreleased. | Normalised the `pubDatetime`/`modDatetime` values to the intended JST moment (converted to UTC) so the post now appears in featured and recent listings. | `src/data/blog/2025/pages-cms-for-my-astro-blog-and-quartz.md` |
