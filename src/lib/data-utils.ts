...
export const getPublishDate = (post: CollectionEntry<'blog'>): Date =>
  (post.data.pubDatetime as Date) ?? (post.data.date as Date)

const splitSegments = (id: string) => id.split('/')
const hasYearPrefix = (segments: string[]) =>
  segments.length > 1 && /^\d{4}$/.test(segments[0])
...
const isPostVisible = (
  ...
  const publishAt = getPublishDate(post)
  ...
  return now > publishAt.getTime() - (SITE.scheduledPostMargin ?? 0)
}
...
export async function getAllPosts(): Promise<CollectionEntry<'blog'>[]> {
  ...
  return posts
    .filter((post) => isPostVisible(post) && !isSubpost(post.id))
    .sort((a, b) => getPublishDate(b).valueOf() - getPublishDate(a).valueOf())
}
...
export async function getAdjacentPosts(currentId: string): Promise<{
  ...
  if (isSubpost(currentId)) {
    ...
    const subposts = posts
      ...
      .sort((a, b) => {
        const dateDiff =
          getPublishDate(a).valueOf() - getPublishDate(b).valueOf()
...
export function getParentId(subpostId: string): string {
  const segments = splitSegments(subpostId)
  if (segments.length <= 1) {
    return subpostId
  }

  if (hasYearPrefix(segments)) {
...
export async function getSubpostsForParent(
  ...
  return posts
    ...
    .sort((a, b) => {
      const dateDiff = getPublishDate(a).valueOf() - getPublishDate(b).valueOf()
      if (dateDiff !== 0) return dateDiff
...
export function groupPostsByYear(
  ...
  return posts.reduce(
    (acc: Record<string, CollectionEntry<'blog'>[]>, post) => {
      const year = getPublishDate(post).getFullYear().toString()
      ;(acc[year] ??= []).push(post)
      return acc
    },
    {},
...
export function isSubpost(postId: string): boolean {
  const segments = splitSegments(postId)
  if (segments.length <= 1) {
    return false
  }
